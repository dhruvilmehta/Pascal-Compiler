 ...
 * Runtime text input for Pascal programs, based on the front end
 * scanner.
 * </p>
 */
public class PascalTextIn {
    ...
    static {
        try {
            scanner = new PascalScanner(new Source(new BufferedReader(new InputStreamReader(System.in))));
     ...
     */
    public int readInteger() throws PascalRuntimeException {
        Token token = null;
        try {
     ...
     * @throws PascalRuntimeException if an error occurred.
     */
    public char readChar() throws PascalRuntimeException {
        char ch = ' ';
        try {
            ch = scanner.nextChar();
            ...
            throw new PascalRuntimeException("Read error: invalid character value: '" + ch + "'");
        }
    }
    ...
    private Number parseNumber(Token token, boolean isInteger)
        ...
        if (tokenType == INTEGER) {
            ...
            return isInteger ? value : Float.valueOf(((Integer) value).intValue());
        }
        // Real value.
        else if (tokenType == REAL) {
            Number value = sign == MINUS ? -((Float) token.getValue())
                    : (Float) token.getValue();
            return isInteger ? Integer.valueOf(((Float) value).intValue()) : value;
        }
        // Bad input.
        else {
            throw new Exception();
